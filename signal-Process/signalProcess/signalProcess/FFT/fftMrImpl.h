 ...
 */
template<typename T>
void fftMr<T>::bitReverse( vector<int> &bitrev )
{
    int i, j;
	bitrev[0] = 0;
 ...
 */
template<typename T>
void fftMr<T>::radix2( int nthpo, complex<T> *c0, complex<T> *c1 )
{
    int     k, kk;
    T   *cr0, *ci0, *cr1, *ci1, r1, fi1;
 ...
 */
template<typename T>
void fftMr<T>::radix4( int nthpo, complex<T> *c0, complex<T> *c1,
                          complex<T> *c2, complex<T> *c3 )
{
    int     k, kk;
    T    *cr0, *ci0, *cr1, *ci1, *cr2, *ci2, *cr3, *ci3;
    T    r1, r2, r3, r4, i1, i2, i3, i4;
 ...
 */
template<typename T>
void fftMr<T>::radix8( int nxtlt, int nthpo, int length,
                          ...
                          complex<T> *cc6, complex<T> *cc7 )
{
    ...
    T    *cr0, *cr1, *cr2, *cr3, *cr4, *cr5, *cr6, *cr7;
    T    *ci0, *ci1, *ci2, *ci3, *ci4, *ci5, *ci6, *ci7;
...
/**
 * This routine replaces the input Complex<T> vector by its
 * finite discrete Fourier transform if direction == FORWARD.
 * It replaces the input Complex<T> vector by its finite discrete
 * inverse Fourier transform if direction == INVERSE.
 */
template<typename T>
void fftMr<T>::dft( int direction, int n, complex<T> *b )
{
    int     i, j;
 ...
 */
template<typename T>
void fftMr<T>::fft( vector<complex<T> > &xn )
{
    int nFFT = xn.size();
    if( isPower2(nFFT)  )
        dft( FORWARD, nFFT, reinterpret_cast<Complex<T>*>(xn.begin()) );
    else
        cerr << "The length of signal must abe power of 2!" ;
}

template<typename T>
void fftMr<T>::fft( const vector<T> &xn, vector< complex<T> > &Xk )
{
    int nFFT = xn.size();
    if( isPower2(nFFT)  )
    {
        ...
        dft( FORWARD, nFFT, reinterpret_cast<complex<T>*>(Xk.begin()) );
    }
    else
        cerr << "The length of signal must abe power of 2!" ;
}
 ...
 */
template<typename T>
void fftMr<T>::ifft( vector<complex<T> > &Xk )
{
    int nFFT = Xk.size();
    if( isPower2(nFFT)  )
        dft( INVERSE, nFFT, reinterpret_cast<complex<T>*>(Xk.begin()) );
    else
        cerr << "The length of signal must abe power of 2!" ;
}

template<typename T>
void fftMr<T>::ifft( const vector< complex<T> > &Xk, vector<T> &xn )
{
    int nFFT = Xk.size();
    vector< complex<T> > tmp(Xk);
    if( isPower2(nFFT)  )
    {
        dft( INVERSE, nFFT, reinterpret_cast<Complex<T>*>(tmp.begin()) );
